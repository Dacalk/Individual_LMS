...
// Get all users for new message (primarily teachers and admin)
$users_query = $conn->prepare("SELECT user_id, CONCAT(first_name, ' ', last_name) as name, role, email FROM users WHERE user_id != ? AND status = 'active' ORDER BY role, first_name ASC");
$users_query->bind_param("i", $_SESSION['user_id']);
$users_query->execute();
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div style="display: flex; gap: 10px; margin-bottom: 20px; border-bottom: 2px solid var(--border-color);">
                        <button class="tab-button active" onclick="switchTab('inbox')" id="inboxTab" style="padding: 12px 24px; border: none; background: none; cursor: pointer; font-weight: 600; border-bottom: 3px solid #4F46E5; color: #4F46E5;">
...
function switchTab(tab) {
    ...
    if (tab === 'inbox') {
        ...
        document.getElementById('inboxTab').style.borderBottom = '3px solid #4F46E5';
        document.getElementById('inboxTab').style.color = '#4F46E5';
    } else {
        document.getElementById('sentContent').style.display = 'block';
        document.getElementById('sentTab').classList.add('active');
        document.getElementById('sentTab').style.borderBottom = '3px solid #4F46E5';
        document.getElementById('sentTab').style.color = '#4F46E5';
    }
}
