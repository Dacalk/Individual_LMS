...
// Classes teacher can manage (subject teacher or class teacher)
...
$allowed_class_ids = array_column($teacher_classes, 'class_id');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'create') {
        ...
        if (!$error && $class_id !== null && !in_array($class_id, $allowed_class_ids, true)) {
            $error = 'You do not have permission to create a quiz for the selected class.';
        }
        ...
        $toggle_stmt = $conn->prepare("UPDATE mcq_quizzes SET status = ? WHERE quiz_id = ? AND created_by = ?");
        $toggle_stmt->bind_param("sii", $new_status, $quiz_id, $_SESSION['user_id']);
        ...
        $quiz_id = (int)$_POST['quiz_id'];
        $delete_stmt = $conn->prepare("DELETE FROM mcq_quizzes WHERE quiz_id = ? AND created_by = ?");
        $delete_stmt->bind_param("ii", $quiz_id, $_SESSION['user_id']);
...
$quiz_list_stmt = $conn->prepare("SELECT q.*, c.class_name, c.section,
...
$quiz_list_stmt->bind_param("i", $_SESSION['user_id']);
...
<div class="dashboard-wrapper">
    ...
    <div class="main-content">
        ...
        <div class="content">
                ...
                <div class="card-body">
                    <form method="POST" class="quiz-form">
                        ...
                        <div class="form-group">
                            ...
                            <select name="class_id" class="form-control" <?php echo empty($teacher_classes) ? 'disabled' : ''; ?>>
            ...
            <div class="card">
                <div class="card-header">
                    <h3><i class="fas fa-list"></i> My Quizzes</h3>
                </div>
                <div class="card-body">
